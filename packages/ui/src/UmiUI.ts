...
export default class UmiUI {
  ...
  constructor() {
    ...
    if (process.env.CURRENT_PROJECT) {
      const key = this.config.addProjectWithPath(
        join(process.cwd(), process.env.CURRENT_PROJECT)
      );
    ...
    if (!this.developMode && service) {
      ...
      if (process.env.UI_CHECK_LOCAL !== "none" && localBin) {
        const { version } = JSON.parse(
          readFileSync(join(cwd, "node_modules", pkgModule), "utf-8")
        );
        if (
          !semver.gt(
            version,
            process.env.BIGFISH_COMPAT ? "2.23.0" : "2.12.0-beta.2"
          )
        ) {
          throw new ActiveProjectError({
  ...
  getBasicAssets() {
    const script = this.basicUIPath
      ? getBasicScriptContent(this.basicUIPath)
      : "";
    return {
      script
    };
  ...
  async installDeps(
    npmClient,
    projectPath,
    { onProgress, onSuccess, taobaoSpeedUp }
  ) {
  ...
  handleCoreData(
    ...
    { log, send, success, failure, progress }
  ) {
    switch (type) {
      ...
      case "@@project/open":
        try {
          ...
          console.error(
            chalk.red(`Error: Attach Project of key ${payload.key} FAILED`)
          );
          console.error(e);
          failure(
            pick(e, ["title", "message", "stack", "actions", "exception"])
          );
        }
        break;
          ...
          data: [
            {
              ...
              description:
                "A layout-only ant-design-pro boilerplate, use together with umi block",
      ...
      case "@@project/detectLanguage":
        try {
          assert(
            key && this.servicesByKey[key],
            `Detect language failed, key must be supplied.`
          );
          const service = this.servicesByKey[key];
          success({
  ...
  async start() {
    return new Promise(async (resolve, reject) => {
      ...
      function normalizeHtml(html, scripts) {
        ...
        html = html.replace(
          "<body>",
          `<body>\n<script>window.g_umi = { version: "${process.env
            .UMI_VERSION || ""}"};</script>`
        );
        if (process.env.BIGFISH_COMPAT) {
          html = html.replace(
            "<body>",
            `<body>\n<script>window.g_bigfish = { version: "${process.env
              .BIGFISH_VERSION || ""}" };</script>`
          );
        }
      ...
      function formatLogMessage(message) {
        let ret =
          message.length > 500
            ? `${message.slice(0, 500)} ${chalk.gray("...")}`
            : message;
        ret = ret.replace(
          /{"type":"(.+?)"/,
          `{"type":"${chalk.magenta.bold("$1")}"`
        );
        return ret;
      }
      ...
      ss.on("connection", conn => {
        ...
        conn.on("data", message => {
          try {
            const { type, payload, $lang: lang, $key: key } = JSON.parse(
              message
            );
            ...
            if (type.startsWith("@@")) {
              ...
              assert(
                this.servicesByKey[key],
                `service of key ${key} not exists.`
              );
              const service = this.servicesByKey[key];
            ...
            if (this.basicConfigPath) {
              const { services } =
                // eslint-disable-next-line import/no-dynamic-require
                require(this.basicConfigPath).default ||
                require(this.basicConfigPath) ||
                {};
