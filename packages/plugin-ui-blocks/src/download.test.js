...
describe("test block download utils", () => {
  ...
  it("parseGitUrl", async () => {
    ...
    expect(
      await parseGitUrl("git@gitlab.alitest-inc.com:bigfish/bigfish.git", true)
    ).toEqual({
    ...
    expect(
      await parseGitUrl("http://gitlab.alitest-inc.com/bigfish/hello", true)
    ).toEqual({
    ...
    expect(
      await parseGitUrl("https://github.com/umijs/umi-blocks.git", true)
    ).toEqual({
      repo: "https://github.com/umijs/umi-blocks.git",
      branch: "master",
      path: "/",
    ...
    expect(
      await parseGitUrl(
        "http://192.169.3.19/YYJay/test-umi-block/tree/master/demo",
        true
      )
    ).toEqual({
  ...
  it("getParsedData with defaultGitUrl", async () => {
    const args = await getParsedData(
      "https://github.com/test/name/tree/somebranch/demo",
      {
