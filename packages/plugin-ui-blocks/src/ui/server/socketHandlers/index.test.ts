...
describe("block interface socketHandlers test", () => {
  describe("org.umi.block.checkIfCanAdd", () => {
    it("约定式路由报错", () => {
      ...
      expect(params2.failure.mock.calls[0][0].message).toMatch(
        /不支持约定式路由/
      );
    });
    ...
    it("Bigfish", () => {
      ...
      expect(params4.failure.mock.calls[0][0].message).toMatch(
        /请开启 dva 配置/
      );
