...
export default function({ success, payload, api, lang, failure }) {
  ...
  const configRoutes =
    Array.isArray(api.config.routes) && api.config.routes.length > 0;
  // 不支持约定式路由
  if (!configRoutes) {
    failure({
      message:
  ...
  const payloadType =
    (payload as { type: string }).type === "block" ? "区块" : "模板";
  ...
  const bigfishLocaleEnable =
    api.config.locale && api.config.locale.enable !== false;

  const checkConfigRules = {
    dva: {
      enable: isBigfish
        ? api.config.dva !== false
        : reactPlugin && reactPluginOpts.dva,
      message: {
        "zh-CN": isBigfish
    ...
    i18n: {
      enable: isBigfish
        ? !!bigfishLocaleEnable
        : reactPlugin &&
          reactPluginOpts.locale &&
          reactPluginOpts.locale.enable !== false,
      message: {
        "zh-CN": isBigfish
  ...
  Object.keys(checkConfigRules).forEach(rule => {
    if (
      haveFeature(rule) &&
      checkConfigRules[rule] &&
      !checkConfigRules[rule].enable
    ) {
      failure({
        message:
          checkConfigRules[rule].message[lang] ||
          checkConfigRules[rule].message["zh-CN"]
      });
      return false;
    }
  });
